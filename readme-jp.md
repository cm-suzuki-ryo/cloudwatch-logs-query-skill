...
1. **実環境検証済みの内容のみ掲載** — すべてのクエリパターン・関数の挙動は、実際のAWS環境で動作確認を行った上で記載しています。
2. **更新時は必ず動作確認を実施** — AWS公式ドキュメントやネット上の情報であっても、実環境でクエリを実行して結果を確認してから反映します。
3. **動作不良の内容は積極的に除外** — 動作しない関数（例: `ago()`）やサイレント失敗するパターン（例: ISO 8601文字列でのタイムスタンプ比較）は、誤用防止のためスキルから除外し、Common Pitfallsとして「使えない」ことを明記します。
...
- https://dev.classmethod.jp/articles/cloudwatch-logs-insights-new-commands-functions-2026-june/ （6月分）
